...
public class RandomFacingOreFeature extends OreFeature {
    public RandomFacingOreFeature(Codec<OreFeatureConfig> codec) {
        super(codec);
    }
                                    ...
                                    if (ai * ai + ak * ak + am * am < 1.0D) {
                                        ...
                                        if (!bitSet.get(an)) {
                                            ...
                                            if (config.target.test(world.getBlockState(mutable), random)) {
                                                world.setBlockState(mutable, config.state.with(Properties.FACING, Direction.random(random)), 2);
                                                ++j;
                                            }
                                        }
                                    }
